    ...
    try:
                ...
                if global_filters:
                    ...
                    try:
                        ...
                        if traffic_overview:
                            ...
                            # Previous-period data is already in traffic_overview (prev_* keys).
                            # get_traffic_overview now applies the same global_filters to all 4 GA4 requests.
        ...
        try:
            ...
            if campaign_links:
                ...
                # Only count keywords still present on the Agency Analytics platform
                # (is_active); deleted keywords are excluded so KPIs match the charts.
        ...
        if chart_ga4_property_id:
            try:
                ...
                # Run all 4 chart calls concurrently — they are fully independent
                _chart_results = await asyncio.gather(
...
async def get_client_keyword_rankings_over_time(
    ...
    try:
        ...
        # NOTE: deliberately NOT filtered on volume — see the comment on the live copy of this
        # handler in app/api/routes/keywords_data.py. Agency Analytics counts every tracked
        # keyword regardless of search volume, and returns no volume at all for many keywords.
